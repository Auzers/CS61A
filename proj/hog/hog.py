...
def boar_brawl(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 2
    unit_digit = player_score % 10
    tens_digit = opponent_score // 10 % 10
    res = abs(unit_digit - tens_digit) * 3
    if res > 1:
        return res
    else:
        return 1
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    add = 0
    if num_rolls == 0:
        add += boar_brawl(player_score,opponent_score)
    else:
        add += roll_dice(num_rolls,dice)
    return add
    # END PROBLEM 3
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    while(curr <= 10):
        ...
        currnum = averaged_dice(curr,dice)
        if(currnum > max):
            max = currnum
            index = curr
        curr += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    num = score + boar_brawl(score,opponent_score)
    num = sus_points(num)
